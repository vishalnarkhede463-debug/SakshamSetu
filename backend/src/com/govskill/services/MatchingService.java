...
public class MatchingService {
    ...
    public static SkillGapResult getSkillGap(StudentProfile student, Opportunity opp) {
        SkillGapResult res = new SkillGapResult();
        if (opp == null || opp.getSkills() == null) return res;

        List<String> reqTokens = tokenize(opp.getSkills());
        List<Skill> studentSkills = student != null ? student.getSkillsList() : null;
        ...
        for (String req : reqTokens) {
            ...
            if (matched) {
                res.matchedSkills.add(req);
            } else {
                res.missingSkills.add(req);
            }
        }

        if (!reqTokens.isEmpty()) {
            res.matchPercentage = (int) Math.round(((double) res.matchedSkills.size() / reqTokens.size()) * 100);
        }
        return res;
    }
