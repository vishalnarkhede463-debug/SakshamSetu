...
const A11Y = {
  ...
  initAccessiblePortalMode() {
    // 1. Wire Speech Speed Select
    const rateSelect = document.getElementById('a11y-select-speech-rate');
    if (rateSelect) {
      rateSelect.value = this.settings.speechRate.toString();
      rateSelect.onchange = (e) => {
        this.settings.speechRate = parseFloat(e.target.value);
        this.saveSettings();
        this.speak(`Speech speed set to ${e.target.options[e.target.selectedIndex].text}`, true);
      };
    }

    // 2. Wire Theme / Contrast Select
    const contrastSelect = document.getElementById('a11y-select-contrast');
    if (contrastSelect) {
      contrastSelect.value = this.settings.contrastMode;
      contrastSelect.onchange = (e) => {
        this.settings.contrastMode = e.target.value;
        this.applySettings();
        this.saveSettings();
        this.speak(`Visual theme updated to ${e.target.options[e.target.selectedIndex].text}`, true);
      };
    }

    // 3. Wire Font Size Select
    const fontSelect = document.getElementById('a11y-select-font-size');
    if (fontSelect) {
      fontSelect.value = this.settings.fontSize;
      fontSelect.onchange = (e) => {
        this.settings.fontSize = e.target.value;
        this.applySettings();
        this.saveSettings();
        this.speak(`Text scale updated to ${e.target.options[e.target.selectedIndex].text}`, true);
      };
    }

    // 4. Wire Speech Toggle Button
    const speechBtn = document.getElementById('a11y-btn-toggle-speech');
    if (speechBtn) {
      speechBtn.onclick = () => {
        this.toggleSpeech();
      };
    ...
    // 5. Wire Voice Commands Button
    const voiceBtn = document.getElementById('a11y-btn-voice-commands');
    if (voiceBtn) {
      voiceBtn.onclick = () => {
  ...
  executeVoiceCommand(cmd) {
    ...
    } else if (cmd.includes('generate resume') || cmd.includes('build resume') || cmd.includes('resume generator')) {
      this.speak("Opening Official ATS Resume Generator.", true);
    ...
    } else if (cmd.includes('skill hub') || cmd.includes('learn skill') || cmd.includes('courses')) {
      this.speak("Opening Skill Development Hub.", true);
      window.location.href = 'skill-hub.html';
    } else if (cmd.includes('assessment') || cmd.includes('certification') || cmd.includes('certificate')) {
      this.speak("Opening Certification and Assessment Center.", true);
      window.location.href = 'assessment-center.html';
    } else if (cmd.includes('practice exam') || cmd.includes('mock test') || cmd.includes('exam')) {
      this.speak("Opening JEE and MHT-CET practice exam simulator.", true);
      window.location.href = 'practice-exam.html';
    } else if (cmd.includes('interview practice') || cmd.includes('voice interview') || cmd.includes('mock interview')) {
      this.speak("Opening Voice-Based AI Interview practice room.", true);
      window.location.href = 'voice-interview-practice.html';
