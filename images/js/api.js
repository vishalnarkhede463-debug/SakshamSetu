...
const API = {
  ...
  async route(method, endpoint, body = {}, params = {}) {
    ...
    if (clean === '/student/skills') {
      ...
      if (method === 'POST') {
        const newSkill = {
          ...
          verified: false // Explicit rule: skills cannot be verified without uploaded certificate!
        };
    ...
    if (clean === '/student/certificates') {
      ...
      if (method === 'POST') {
        ...
        if (skillId) {
          ...
          if (sk) {
            skillName = sk.name;
            sk.verified = true; // Rule: certificate verifies the skill!
            LocalStore.set('skills', allSkills);
          }
        }
    ...
    if (clean.startsWith('/student/certificates/')) {
      ...
      // If removed, check if skill should revert to unverified
    ...
    // 6. EMPLOYER OPERATIONS & CANDIDATE SEARCH (STRICT PRIVACY ENFORCEMENT)
    if (clean === '/employer/candidates') {
      ...
      // Only show profiles where consentGiven = true AND profileVisibility = true!
